...
{
    name = "Неизвестно";
    capital = "Нет"; // ПРЯМОЕ ПРИСВАИВАНИЕ, НЕ strcpy
    foundationDate = 0;
    area = 0;
    population = 0;
...
{
    name = n;
    capital = c; // ПРЯМОЕ ПРИСВАИВАНИЕ, НЕ strcpy
    foundationDate = date;
    area = a;
    population = pop;
    cities = cityList;
...
{
    name = other.name;
    capital = other.capital; // ПРЯМОЕ ПРИСВАИВАНИЕ, НЕ strcpy
...
string Country::getCapital() const { return capital; } // ПРОСТО ВОЗВРАЩАЕМ
...
{
    capital = newCapital; // ПРОСТО ПРИСВАИВАЕМ, БЕЗ strcpy
}
