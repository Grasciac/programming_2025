...
{ // функция мейн
    ...
    /*
    не использую signed int потому что сторона треугольника не может быть отрицательной
    можно было уменьшить количество строк если использовать:
    std::cout <<"Тут текст"; std:: cin >> a;
    */
    std::cin >> a;
    ...
    std::cout << "MIN double значение = " << std::numeric_limits<double>::lowest() << std::endl; // др константа
